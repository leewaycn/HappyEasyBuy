...
@interface HEBSearchViewController ()<QMUITextFieldDelegate>

/** 搜索按钮 */
@property(nonatomic, strong) QMUITextField *searchField;

@property(nonatomic, strong) HEBSearchView *searchView;

@end

@implementation HEBSearchViewController
...
- (void)loadSearchView {
    HEBSearchView *searchView = [[HEBSearchView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
...
- (void)searchDidClick {
    ...
    [Networking postUrl:Foodslist params:@{@"index_page":@"1", @"index_size":@"1000", @"lng":getLon, @"lat":getLat, @"city_id":getCity_id, @"name":self.searchField.text.removeSpace} callBack:^(id response, MainModel *mainModel, NSError *errorData) {
        [weakSelf dismissProgressHUD];
        weakSelf.searchView.foodStoresModelArr = [NSArray yy_modelArrayWithClass:[HEBFoodStoresModel class] json:mainModel.data];
